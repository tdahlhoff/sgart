...
/**
 * Orchestrates {@link UpdateItem} (AC3, AC5, AC8): resolve the caller's household-scoped {@code
 * MemberId} (AD-2), load the {@link ShoppingList} aggregate, and let it enforce the {@code
 * Open}-only, unknown-item, and collision invariants. The append uses the <em>loaded</em> stream
 * version as the expected version (AD-8); a no-change update raises nothing, so the append is
 * skipped (convergent no-op, mirroring {@link RenameShoppingListHandler}).
 */
public final class UpdateItemHandler {

    private final EventStore eventStore;
    private final ResolveMemberIdentity resolveMemberIdentity;

    public UpdateItemHandler(EventStore eventStore, ResolveMemberIdentity resolveMemberIdentity) {
    ...
    public void handle(
        ...
        UpdateItem command = new UpdateItem(listId, itemId, name, note, quantity, commandId, loadedVersion);

        try {
            list.updateItem(command.itemId(), command.name(), command.note(), command.quantity(), command.commandId());
