...
/**
 * Orchestrates {@link CheckOffItem} (Story 3.3, AC1): resolve the caller's household-scoped
 * {@code MemberId} (AD-2), load the {@link ShoppingList} aggregate, and let it enforce the
 * {@code IN_TRIP}-only and unknown-item invariants. A convergent no-op (already DONE) raises
 * nothing; the append is skipped in that case.
 */
public final class CheckOffItemHandler {

    private final EventStore eventStore;
    private final ResolveMemberIdentity resolveMemberIdentity;

    public CheckOffItemHandler(EventStore eventStore, ResolveMemberIdentity resolveMemberIdentity) {
    ...
    public void handle(
        ...
        CheckOffItem command = new CheckOffItem(listId, itemId, commandId, loadedVersion);

        try {
            list.checkOffItem(command.itemId(), command.commandId());
