...
/**
 * Orchestrates {@link AssignItemToStore} (AC1, AC5, AC7): resolve the caller's household-scoped
 * {@code MemberId} (AD-2), load the {@link ShoppingList} aggregate, and let it enforce the {@code
 * Open}-only, unknown-item, and same-store-no-op invariants. The append uses the <em>loaded</em>
 * stream version as the expected version (AD-8); a same-store no-op raises nothing, so the append
 * is skipped (convergent no-op, mirroring {@link UpdateItemHandler}).
 */
public final class AssignItemToStoreHandler {

    private final EventStore eventStore;
    private final ResolveMemberIdentity resolveMemberIdentity;

    public AssignItemToStoreHandler(EventStore eventStore, ResolveMemberIdentity resolveMemberIdentity) {
    ...
    public void handle(
        ...
        AssignItemToStore command = new AssignItemToStore(listId, itemId, storeId, commandId, loadedVersion);

        try {
            list.assignItemToStore(command.itemId(), command.storeId(), command.commandId());
