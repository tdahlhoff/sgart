...
/**
 * Orchestrates {@link DemoteMember} (AC2, AC4, AC5): resolve the caller, let {@link Household}
 * enforce the Admin-only gate and the last-Admin invariant, append. A role change, not a membership
 * removal — <strong>no ACL de-link</strong> (unlike {@link RemoveMemberHandler}).
 */
public final class DemoteMemberHandler {

    private final EventStore eventStore;
    private final ResolveMemberIdentity resolveMemberIdentity;

    public DemoteMemberHandler(EventStore eventStore, ResolveMemberIdentity resolveMemberIdentity) {
     ...
     * @throws LastAdminApplicationException if the target is the household's last Admin (409, AC5)
     */
    public void handle(String keycloakUserId, String rawHouseholdId, String rawTargetMemberId, String rawCommandId) {
        ...
        DemoteMember command = new DemoteMember(householdId, targetMemberId, commandId, loadedVersion);

        try {
            household.demoteMember(callerMemberId, command.targetMemberId(), command.commandId());
