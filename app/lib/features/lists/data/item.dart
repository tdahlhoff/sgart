...
/// the item's in-trip lifecycle (`"OPEN"`, `"DONE"`, `"POSTPONED"`) — `"OPEN"` is the default /
/// birth state; the server always sends it once Story 3.3 is live.
class Item {
  const Item({
  ...
  /// value is malformed. A missing `status` defaults to `"OPEN"` for forward-compat with older
  /// server versions; a non-string, non-null value is malformed.
