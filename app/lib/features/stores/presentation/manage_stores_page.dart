...
/// suggestion inline (accept / clear), and a per-row remove that **archives** (with helper copy
/// explaining past purchases are kept). Creates its own [StoresCubit] over the [StoresApi] and
/// [StoreChainReferenceCache] provided up the tree (AC4: the same reusable creation path later
/// pickers mount).
class ManageStoresPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const _ManageStoresView(),
    );
  }
}

class _ManageStoresView extends StatefulWidget {
  const _ManageStoresView();

  @override
  State<_ManageStoresView> createState() => _ManageStoresViewState();
}

class _ManageStoresViewState extends State<_ManageStoresView> {
