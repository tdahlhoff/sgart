...
- **Rule:** Erasure = destroy the person's Identity-ACL mapping rows + scrub PostgreSQL read models + purge device/offline caches + delete the Keycloak account. (Story 7.1: the erasure checklist also covers the `ProvisionedAccount` shell row — a pseudonymous `keycloakUserId` + timestamp, no PII — deleted alongside the Keycloak account; Epic 6 owns wiring this into the erasure use case. The retention sweep, `SweepNeverActivatedAccounts`, already deletes it independently for the *never-activated* case, storage limitation rather than erasure.) The event log is **never** rewritten; residual orphaned `MemberId`s are unlinkable and thus anonymized (GDPR Recital 26). Any content that is itself PII inside an event/blob (e.g. receipt images / OCR text, invite email) must instead use **crypto-shredding** — a per-subject data key in a vault, destroyed on erasure. Crypto-shredding is a **general mechanism and a hard gate before the public/live release** (ADR-0001), no longer a Post-MVP receipts-only detail: on completion it retires the invite email side-store (AD-6) and converts the AD-5 identity mapping into a rebuildable projection, deleting the invite handlers' out-of-band-write compensation. Deferred through beta — safe because beta data is not migrated to live, so there is no back-fill/migration problem — and built alongside the Epic 6 erasure story, which shares the machinery.
...
- **Crypto-shredding implementation** — the pattern is fixed (AD-7, ADR-0001) and is now a **general mechanism gated before the public/live release**, not Post-MVP-receipts-only. Deferred through beta (beta data is not migrated to live, so no back-fill problem); the key-vault mechanics are settled when it is built alongside the Epic 6 erasure story. The invite email side-store and its out-of-band-write compensation are retired earlier — by Epic 7/Story 7.5 (invites carry no email for beta) — so that is no longer part of crypto-shredding's scope (rev E).
...
- **Production hosting target is fixed (ADR-0002):** a `netcup VPS Lite 2 G12s` (4 vCore x86/KVM, **8 GB RAM**, 160 GB SSD, EU auto-location) hosts the five-service stack **plus a co-located Murmur server** (2–4 people). 8 GB is the hard sizing floor → JVM heap caps (backend + Keycloak) + ~2 GB swap. The production-deployment work owed against this — sign the netcup AVV, prod Compose that drops the DEV-only flags (`KURRENTDB_INSECURE`, Keycloak `start-dev`), encryption at rest (LUKS for Postgres + KurrentDB), TLS reverse proxy (never expose KurrentDB/Postgres/Keycloak; SSE-safe), encrypted EU backups + retention, server hardening (SSH key-only, firewall 443/TCP + 64738 TCP+UDP, fail2ban, auto updates) — is enumerated in ADR-0002.
- **Geolocation store discovery (FR-28)** — gated on a privacy-friendly geodata source (self-hosted OSM/Overpass/Nominatim); backlog.
- **Frictionless account provisioning (FR15/Epic 7) — required for beta:** account creation is backend-mediated (Keycloak Admin API); login is browserless and app-direct via a **custom Direct-Grant authenticator SPI** that verifies a device-signed challenge (Story 7.0) — no browser surface and no password ROPC. `sgart-app` keeps `directAccessGrantsEnabled: true` with a custom Direct Grant flow bound; `sgart-admin`'s service account gains `manage-users` (currently `view-users` only) and a real secret (the checked-in dev placeholder cannot ship) — **no** impersonation/token-exchange permission (impersonation token exchange is legacy `token-exchange:v1`, Preview + Deprecated in 26.7, and was rejected; Standard Token Exchange v2 is GA but cannot change subject). A new unauthenticated `identity` provisioning endpoint (added to `SecurityConfig` permit list) backed by a `ProvisionAccount` command/handler and one `adapter.out` (`KeycloakAdminCreateAccount`, registering the device public key) following the `KeycloakAdminFindHouseholdMemberByEmail` (Story 4.6) pattern; plus the custom authenticator SPI as a Keycloak-deployed artifact. No new domain event or aggregate — AD-5/AD-6/AD-7 untouched. New NFRs: the create-account endpoint is IP-based rate-limited for beta (device attestation a fast-follow), and never-activated accounts are retention-swept (GDPR storage limitation). The opt-in "recover by email" path (Story 7.3) adds a beta infra dependency — transactional-email/SMTP (SPF/DKIM) on the netcup stack (ADR-0002), isolated to that feature and using no Keycloak-hosted page; the attached email is personal data covered by erasure/export. The `collaboration` context's invite email side-store (`EmailHmac`, `InviteEmailSideStore`) is retired — invites carry no email for beta (join code / link only, Story 7.5).
