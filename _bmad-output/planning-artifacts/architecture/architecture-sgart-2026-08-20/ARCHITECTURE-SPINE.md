...
- **Crypto-shredding implementation** — the pattern is fixed (AD-7, ADR-0001) and is now a **general mechanism gated before the public/live release**, not Post-MVP-receipts-only. Deferred through beta (beta data is not migrated to live, so no back-fill problem); the key-vault mechanics are settled when it is built alongside the Epic 6 erasure story. On completion it retires the invite email side-store and the invite handlers' out-of-band-write compensation.
...
- **Public-phase concerns:** self-hosted push (UnifiedPush) vs. FCM/APNs, full web client, multi-market StoreChain sourcing, additional UI languages. **Self-registration (FR15/Epic 7) is scheduled, not open-ended**: flip `registrationAllowed`/`resetPasswordAllowed` in the Keycloak realm, surface a "Register" entry point on the app's own login screen (opened in-app via the existing AppAuth surface), and use Keycloak's built-in **magic-link/email-OTP authentication flow** for first-time auth (both the no-inviter self-register path and invite-acceptance for a brand-new invitee) followed by a mandatory **"Update Password" required action** — the magic-link email may since be deleted, so it cannot be the recurring credential — plus the "Terms and Conditions" required action for consent. No new domain event or aggregate — this happens entirely inside Keycloak before any SGART command is issued, so AD-5/AD-6/AD-7 are untouched.
