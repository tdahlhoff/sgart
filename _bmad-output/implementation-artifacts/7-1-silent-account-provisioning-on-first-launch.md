...
| 2026-09-14 | Timo decided on the flagged sign-out question: remove it entirely (not just relabel/redesign). Investigation surfaced the removal was overdue for a second reason beyond privacy-perception — `AuthCubit.bootstrap()` only runs once at app start, so `signOut()` had no way to retrigger sign-in, meaning "Abmelden" actually stranded the app on an infinite loading spinner with no recovery short of a force-quit. Removed all 3 sign-out affordances (Profil tab, the two pre-household app-bar icons) and `AuthCubit.signOut()`, plus what existed only to support it: the `_sessionGeneration` race guard in `tryRefreshTokens()`/push-token registration, `OidcClient.endSession` (and the now-dead `KeycloakConfig.logoutEndpoint` it was the only caller of), and `AuthCubit`'s now-unused `ActiveHouseholdStore` dependency — the port's `clear()` method itself stays for a plausible future caller (Story 7.2/7.3's account-recovery import), as does `PushNotifications.unregister()` (already an unwired Story 4.5 contract). `LocaleAuthBridge`'s `unauthenticated`-triggered locale reset was deliberately left untouched (a separate Story 1.10 concern) — its one test was rewired to drive the state directly via `_ControllableAuthCubit` rather than the removed `signOut()`, so that behavior stays covered even though nothing currently triggers it in production. `flutter analyze` clean; `flutter test` 665/665 (down from 674 — the removed sign-out coverage). Committed `e66dccb`. | Dev Agent (Claude Sonnet 5) |
