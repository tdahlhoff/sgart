...
- **MemberId** — The opaque, per-Household pseudonym used in every event and read model in place of any real identity. The Identity ACL is its sole issuer and holds the only mapping back to a Keycloak account. (Never `keycloakUserId`, email, or name in the domain.)
